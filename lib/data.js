...
export async function getGamesWithTodayResults() {
  ...
  return games.map((game) => ({
    ...game,
    first: formatResult(resultMap.get(`${String(game._id)}:${yesterday}`)?.result || "XX"),
    second: formatResult(resultMap.get(`${String(game._id)}:${today}`)?.result || "XX"),
    firstUpdatedAt: resultMap.get(`${String(game._id)}:${yesterday}`)?.updatedAt || resultMap.get(`${String(game._id)}:${yesterday}`)?.createdAt || null,
    secondUpdatedAt: resultMap.get(`${String(game._id)}:${today}`)?.updatedAt || resultMap.get(`${String(game._id)}:${today}`)?.createdAt || null
  }));
}
