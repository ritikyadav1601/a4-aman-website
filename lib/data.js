...
export async function getGamesWithTodayResults() {
  ...
  if (games.some(isCityGame)) {
    try {
      await connectDB();
      const rows = await getDailyNumberRowsForDates(games, [today, yesterday]);
      ...
      resolvedGames = games.map((game) => {
        ...
        return {
          ...game,
          first: dailyNumberValue(yesterdayRow) || "XX",
          second: dailyNumberValue(todayRow) || "XX",
          firstUpdatedAt: yesterdayRow?.updatedAt || yesterdayRow?.revealedAt || yesterdayRow?.createdAt || null,
          secondUpdatedAt: todayRow?.updatedAt || todayRow?.revealedAt || todayRow?.createdAt || null
        };
      });
    } catch (error) {
...
export async function getMonthlyRows({ year, month, untilToday = true, games: activeGames } = {}) {
  ...
  if (activeCityGames) {
    try {
      ...
      const byCityDate = dailyNumberMap(await getDailyNumberRowsForRange(games, start, end));
      ...
      for (let day = 1; day <= limit; day++) {
        ...
        for (const game of games) {
          const result = byCityDate.get(`${game.cityId || String(game._id)}:${dateKey}`);
          row[sanitizeColumn(game.name)] = dailyNumberValue(result) || "-";
        }
        rows.push(row);
      }
  ...
  if (hasMongo()) {
    ...
    if (games.some(isCityGame)) {
      try {
        ...
        const byCityDate = dailyNumberMap(await getDailyNumberRowsForRange([game], `${year}-01-01`, `${year}-12-31`));

        for (let month = 1; month <= 12; month++) {
          for (let day = 1; day <= 31; day++) {
            const dateKey = `${year}-${String(month).padStart(2, "0")}-${String(day).padStart(2, "0")}`;
            const value = dailyNumberValue(byCityDate.get(`${game.cityId || String(game._id)}:${dateKey}`));
